...
chunk_t x509_build_subjectAltNames(linked_list_t *list)
{
	...
	while (enumerator->enumerate(enumerator, &id))
	{
		...
		subjectAltNames = chunk_cat_safe("mm", &subjectAltNames, &name);
	}
	enumerator->destroy(enumerator);
...
chunk_t x509_build_crlDistributionPoints(linked_list_t *list, int extn)
{
	...
	while (enumerator->enumerate(enumerator, &cdp))
	{
		...
		distributionPoint = asn1_wrap(ASN1_SEQUENCE, "mm",
					...
					crlIssuer);
		crlDistributionPoints = chunk_cat_safe("mm", &crlDistributionPoints,
										  &distributionPoint);
	}
	enumerator->destroy(enumerator);
...
static chunk_t generate_ts(traffic_selector_t *ts)
{
	...
	if (ts->to_subnet(ts, &net, &minbits))
	{
		...
		chunk_t chunk1 = chunk_from_thing(unused);
		chunk_t chunk2 = chunk_create(net->get_address(net).ptr,
										 (minbits + unused) / BITS_PER_BYTE);

		chunk_t composite = chunk_cat_safe("cc", &chunk1, &chunk2);
	...
	chunk_t chunk1 = chunk_from_thing(unused);
	chunk_t chunk2 = chunk_create(from.ptr,
										 (minbits + unused) / BITS_PER_BYTE);
	...
	chunk_t chunk3 = chunk_from_thing(unused);
	chunk_t chunk4 = chunk_create(to.ptr,
										 (maxbits + unused) / BITS_PER_BYTE);

	chunk_t composite1 = chunk_cat_safe("cc", &chunk3, &chunk4);
					 ...
					 private_key_t *sign_key, int digest_alg)
{
	...
	while (enumerator->enumerate(enumerator, &uri))
	{
		...
		authorityInfoAccess = chunk_cat_safe("mm", &authorityInfoAccess,
										&accessDescription);
	}
	enumerator->destroy(enumerator);
		...
		while (enumerator->enumerate(enumerator, &ts))
		{
			switch (ts->get_type(ts))
			{
				case TS_IPV4_ADDR_RANGE:
					...
					v4blocks = chunk_cat_safe("mm", &v4blocks, &block);
					break;
				case TS_IPV6_ADDR_RANGE:
					block = generate_ts(ts);
					v6blocks = chunk_cat_safe("mm", &v6blocks, &block);
					break;
				default:
					break;
			}
		...
		cert->excluded_names->get_count(cert->excluded_names))
	{
		...
		while (enumerator->enumerate(enumerator, &id))
		{
			...
			permitted = chunk_cat_safe("mm", &permitted, &subtree);
		}
		enumerator->destroy(enumerator);
		if (permitted.ptr)
		{
		...
		{
			subtree = asn1_wrap(ASN1_SEQUENCE, "m", build_generalName(id));
			excluded = chunk_cat_safe("mm", &excluded, &subtree);
		}
		enumerator->destroy(enumerator);
		if (excluded.ptr)
		{
	...
	if (cert->cert_policies->get_count(cert->cert_policies))
	{
		...
		while (enumerator->enumerate(enumerator, &policy))
		{
			...
			certPolicies = chunk_cat_safe("mm", &certPolicies, &chunk);
		}
		enumerator->destroy(enumerator);
	...
	if (cert->policy_mappings->get_count(cert->policy_mappings))
	{
		...
		while (enumerator->enumerate(enumerator, &mapping))
		{
			...
			policyMappings = chunk_cat_safe("mm", &policyMappings, &chunk);
		}
		enumerator->destroy(enumerator);
