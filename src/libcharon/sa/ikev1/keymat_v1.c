	...
	auth_method_t auth, shared_key_t *shared_key)
{
	...
	*((uint64_t*)spi_r.ptr) = id->get_responder_spi(id);
	nonces = chunk_cata_new("cc", &nonce_i, &nonce_r);
	...
	data = chunk_cat_new("cccc", &g_xy, &spi_i, &spi_r, &octet_0);
	...
	chunk_t first = this->skeyid_d;
	data = chunk_cat_new("ccccc", &first, &g_xy, &spi_i, &spi_r, &octet_1);
	...
	chunk_t first1 = this->skeyid_a;
	data = chunk_cat_new("ccccc", &first1, &g_xy, &spi_i, &spi_r, &octet_2);
	...
	data = chunk_cata_new("cc", &g_xi, &g_xr);
	chunk_free(&dh_me);
	...
	chunk_t *encr_i, chunk_t *integ_i, chunk_t *encr_r, chunk_t *integ_r)
{
	...
	*encr_r = *integ_r = *encr_i = *integ_i = chunk_empty;
	...
	chunk_t second = chunk_from_thing(protocol), third = chunk_from_thing(spi_r);
	seed = chunk_cata_new("ccccc", &secret, &second, &third, &nonce_i, &nonce_r);
	...
	chunk_t second1 = chunk_from_thing(protocol), third1 = chunk_from_thing(spi_i);
	seed = chunk_cata_new("ccccc", &secret, &second1, &third1, &nonce_i, &nonce_r);
	...
	signature_scheme_t *scheme)
{
	...
	chunk_t third = chunk_from_thing(spi), fourth = chunk_from_thing(spi_other);
	data = chunk_cat_new("cccccc", &dh, &dh_other, &third, &fourth, &sa_i, &id);
	...
	switch (message->get_exchange_type(message))
	{
		case QUICK_MODE:
		{
			...
			{	/* Hash(2) = prf(SKEYID_a, M-ID | Ni_b | Message after HASH) */
				...
				chunk_t first = chunk_from_thing(mid_n);
				data = chunk_cata_new("cc", &first, n_i);
			}
			else
			{	/* Hash(3) = prf(SKEYID_a, 0 | M-ID | Ni_b | Nr_b) */
				name = "Hash(3)";
				chunk_t second = chunk_from_thing(mid_n);
				data = chunk_cata_new("cccc", &octet_0, &second,
								  n_i, n_r);
				add_message = FALSE;
