	...
	private_ike_sa_manager_t *this, ike_sa_t *ike_sa)
{
	...
	{
		DBG1(DBG_MGR, "unable to change initiator SPI of IKE_SA not owned by "
			 "current thread");
		return FALSE;
	}
	...
	if (get_entry_by_sa(this, ike_sa_id, ike_sa, &entry, &segment) == SUCCESS)
	{
		...
		/* threads waiting for this entry are waiting for the wrong ike_sa_id
		 * and on the wrong segment, so drive them out */
